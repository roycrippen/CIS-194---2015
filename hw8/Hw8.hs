...
-- rewrite with type anotations for helper functions
material_implication' :: (p -> q) <-> (Not p \/ q)
material_implication' = Conj aRight aLeft
  where
    aRight :: (p -> q) -> (Not p \/ q)
    aRight pq =
        case excluded_middle of
            ...
            Right np -> Left np
    aLeft :: Not p \/ q -> p -> q
    aLeft (Left np) p = absurd $ np p
    aLeft (Right q) _ = q

-- Exercise 1 -----------------------------------------
disjunctive_syllogism :: (p \/ q) -> Not p -> q
disjunctive_syllogism (Left p)  np = absurd $ np p
...
transposition = Conj aRight aLeft
    where
      aRight :: (p -> q) -> Not q -> Not p
      aRight = modus_tollens
      aLeft :: (Not q -> Not p) -> p -> q
      aLeft nqnp p =
          case excluded_middle' of
              Left q -> q
              Right nq -> absurd (nqnp nq p)

-- more readable version following hints
transposition' :: (p -> q) <-> (Not q -> Not p)
transposition' = Conj aRight aLeft
    where
      aRight :: (p -> q) -> Not q -> Not p
      aRight = modus_tollens
      aLeft :: (Not q -> Not p) -> p -> q
      aLeft nqnp p = nnp_to_p nnq
          where nnp = p_to_nnp p
                nnq =  modus_tollens nqnp nnp

-- helpers that may be used again
p_to_nnp :: p -> Not (Not p)
p_to_nnp = \p np -> np p

nnp_to_p :: Not (Not p) -> p
nnp_to_p = admit

-- Exercise 4 -----------------------------------------
de_morgan :: Not (p \/ q) <-> (Not p /\ Not q)
de_morgan = Conj aRight aLeft
  where
    aRight :: Not (p \/ q) -> (Not p /\ Not q)
    aRight npoq = Conj (npoq . Left) (npoq . Right)
    aLeft ::  (Not p /\ Not q) -> Not (p \/ q)
    aLeft (Conj np _) (Left x)  = np x
    aLeft (Conj _ nq) (Right y) = nq y
...
even_plus_one  E_Zero   = O_One  -- Base Case: The successor of zero is odd
even_plus_one (E_Rec n) =        -- if S (S n) is even then S (S (S n)) is odd
    O_Rec $ even_plus_one n

odd_plus_one :: Odd n -> Even (S n)
odd_plus_one O_One = odd_plus_one $ even_plus_one E_Zero
odd_plus_one (O_Rec n) = E_Rec $ odd_plus_one n

-- Exercise 8 -----------------------------------------
succ_sum :: Forall n -> Forall m -> S n + S m == S (S (n + m))
